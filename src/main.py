...
frames_df = pd.read_csv("./misc/frames_used.csv")
frame_classes = [row["f_id"] for _, row in frames_df.iterrows()]
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, args, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            running_corrects = 0
            # iterate over data
            for batch_idx, ((inputs, lengths), targets) in enumerate(dataloaders[phase]):
                ...
                running_corrects += (predicted == targets).sum()

                if phase == 'train' and batch_idx % 50 == 0:
                    logging.info('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        (epoch + 1),
                        (batch_idx + 1) * len(targets),
                        dataset_sizes[phase],
            ...
            logging.info('{} Loss: {:.4f} Acc: {:.4f}'.format(phase.capitalize(), epoch_loss, epoch_acc))
...
def main(args):
    ...
    train, val, test = TabularDataset.splits(
        path='./data/', train='exemplars_train.csv',
        validation='exemplars_validation.csv', test='exemplars_test.csv', format='csv',
        skip_header=True,
