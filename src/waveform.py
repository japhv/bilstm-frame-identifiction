...
def plot_wave(instr_family_names, samples_arr, pred, is_correct, sr=16000):
    ...
    if is_correct:
        fig.savefig('./' + str(n) + '_waveform_c.png')
    else:
        fig.savefig('./' + str(n) + '_waveform_inc_' + str(pred) + '.png')
...
def plot_specgram(instr_family_names, samples_arr, pred, is_correct, sr=16000):
    ...
    if is_correct:
        fig.savefig('./' + str(n) + '_specgram_c.png')
    else:
        fig.savefig('./' + str(n) + '_specgram_inc_ ' + str(pred) + '.png')
...
def plot_log_power_spec(instr_family_names, samples_arr, pred, is_correct):
    ...
    if is_correct:
        fig.savefig('./' + str(n) + '_log_power_c.png')
    else:
        fig.savefig('./' + str(n) + '_log_power_inc_' + str(pred) + '.png')
